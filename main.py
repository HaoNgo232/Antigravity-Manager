...
def list_accounts():
    ...
    if not accounts:
        ...
        print("\n" + "="*50)
        info(f"Total {len(accounts)} backup(s):")
        print("="*50)
...
def add_account():
    ...
    name = input("\nEnter account name (leave empty for auto-generation): ").strip()
    email = input("Enter email (leave empty for auto-detection): ").strip()
...
def switch_account_interactive():
    ...
    choice = input("\nEnter account number to switch: ").strip()
...
def cli_mode():
    ...
    add_parser.add_argument("--name", "-n", help="Backup name (optional, auto-generated by default)")
